...
def scrape_electric_shuffle(guests, target_date):
    ...
    try:
        ...
        soup = BeautifulSoup(driver.page_source, "html.parser")
        slots = soup.select('div.sc-imWYAI.kIlSzW.sc-jXbUNg.ixHBos.sc-dtInlm.gqAROr span[data-test="reservation-timeslot-button-description"]')
        if len(slots) == 0:
            scraping_status['progress'] = 'No slots available on Electric Shuffle NYC'
            driver.quit()
            return
        ...
        for slot in slots:
            ...
            # Time
            try:
                time = slot.find_previous_sibling("span").get_text().strip()
            except:
                time = "None"
...
def scrape_electric_shuffle_london(guests, target_date):
    ...
    try:
        driver = Driver(uc=True, headless2=True, no_sandbox=True, disable_gpu=True)
        driver.get(f"https://electricshuffle.com/uk/london/book/shuffleboard?preferedvenue=7&preferedtime=19%3A00&guestQuantity={guests}&date={target_date}")
        ...
        if len(holders) == 0:
            scraping_status['progress'] = 'No slots available on Electric Shuffle London'
            driver.quit()
            return
        ...
        for holder in holders:
            try:
                holder_title = holder.find("div",{"class":"es_booking__availability-header es_font-body--semi-bold"}).get_text().strip()
            except:
                holder_title = "Unknown Venue"
                
            slots = holder.find_all("div",{"class":"es_booking__availability__table-cell__wrapper tns-item"})
            ...
            for slot in slots:
                ...
                # Time
                try:
                    time = slot.find("div",{"class":"es_booking__availability__table-cell es_font--pitch-med"})["name"]
                except:
                    time = "None"
                    
                # Description
                try:
                    desc = slot.find("div",{"class":"es_booking__time_wrapper"}).get_text().replace("\n","").strip()
                except:
                    desc = "None"
...
def scrape_lawn_club(guests, target_date, option="Terrace Gaming Lawns"):
    ...
    try:
        ...
        soup = BeautifulSoup(driver.page_source, "html.parser")
        slots = soup.select('div.sc-iLWXdy.BstKG button[data-test="sr-timeslot-button"]')
...
def scrape_spin(guests, target_date):
    ...
    try:
        ...
        soup = BeautifulSoup(driver.page_source, "html.parser")
        slots = soup.select('div.sc-iLWXdy.dBXvkn button[data-test="sr-timeslot-button"]')
...
def scrape_lucky_strike(guests, target_date):
    ...
    try:
        ...
        url = f"https://www.luckystrikeent.com/location/lucky-strike-chelsea-piers/booking/lane-reservation?date=2025-09-{day_str}T23:00:00.000Z&guestsCount={str(guests)}"
...
def scrape_restaurants(guests, target_date, website, lawn_club_option=None, clays_location=None, puttshack_location=None):
    ...
    try:
        ...
        if website == 'swingers_nyc':
            ...
            option = lawn_club_option or "Terrace Gaming Lawns"
