...
def test_all_scrapers():
    ...
    scrapers_to_test = [
        # NYC Venues
        ("Swingers (Nomad)", lambda: swingers.scrape_swingers(TEST_GUESTS, TEST_DATE)),
        ("Electric Shuffle (Nomad)", lambda: electric_shuffle.scrape_electric_shuffle(TEST_GUESTS, TEST_DATE)),
        ...
        ("SPIN (Flatiron)", lambda: spin.scrape_spin(TEST_GUESTS, TEST_DATE, location='flatiron')),
        ("SPIN (Midtown East)", lambda: spin.scrape_spin(TEST_GUESTS, TEST_DATE, location='midtown')),
        ("Five Iron Golf (Financial District)", lambda: five_iron_golf.scrape_five_iron_golf(TEST_GUESTS, TEST_DATE, location='fidi')),
        ("Five Iron Golf (Flatiron)", lambda: five_iron_golf.scrape_five_iron_golf(TEST_GUESTS, TEST_DATE, location='flatiron')),
        ("Five Iron Golf (Midtown East)", lambda: five_iron_golf.scrape_five_iron_golf(TEST_GUESTS, TEST_DATE, location='grand_central')),
        ...
        ("T-Squared Social (Midtown East)", lambda: tsquaredsocial.scrape_tsquaredsocial(TEST_GUESTS, TEST_DATE)),
        ("Chelsea Piers (Chelsea) (DaySmart)", lambda: daysmart.scrape_daysmart_chelsea(TEST_DATE)),  # Only supports 2 guests, but we'll test the function
        
        # London Venues
        ("Swingers (Oxford Circus)", lambda: swingers.scrape_swingers_uk(TEST_GUESTS, TEST_DATE)),
        ("Electric Shuffle (Canary Wharf)", lambda: electric_shuffle.scrape_electric_shuffle_london(TEST_GUESTS, TEST_DATE)),
        ...
        ("F1 Arcade (St Paul's) - Team Racing", lambda: f1_arcade.scrape_f1_arcade(TEST_GUESTS, TEST_DATE, f1_experience='Team Racing')),
        ("Topgolf (Chigwell)", lambda: topgolfchigwell.scrape_topgolf_chigwell(TEST_GUESTS, TEST_DATE)),
        ("Hijingo (Shoreditch)", lambda: hijingo.scrape_hijingo(TEST_GUESTS, TEST_DATE)),
...
def test_single_scraper(scraper_name):
    ...
    scraper_map = {
        # NYC
        'swingers_nyc': ('Swingers (Nomad)', lambda: swingers.scrape_swingers(TEST_GUESTS, TEST_DATE)),
        'electric_shuffle_nyc': ('Electric Shuffle (Nomad)', lambda: electric_shuffle.scrape_electric_shuffle(TEST_GUESTS, TEST_DATE)),
        ...
        'spin_nyc_flatiron': ('SPIN (Flatiron)', lambda: spin.scrape_spin(TEST_GUESTS, TEST_DATE, location='flatiron')),
        'spin_nyc_midtown': ('SPIN (Midtown East)', lambda: spin.scrape_spin(TEST_GUESTS, TEST_DATE, location='midtown')),
        'five_iron_fidi': ('Five Iron Golf (Financial District)', lambda: five_iron_golf.scrape_five_iron_golf(TEST_GUESTS, TEST_DATE, location='fidi')),
        ...
        'tsquaredsocial_nyc': ('T-Squared Social (Midtown East)', lambda: tsquaredsocial.scrape_tsquaredsocial(TEST_GUESTS, TEST_DATE)),
        'daysmart_chelsea': ('Chelsea Piers (Chelsea)', lambda: daysmart.scrape_daysmart_chelsea(TEST_DATE)),
        
        # London
        'swingers_london': ('Swingers (Oxford Circus)', lambda: swingers.scrape_swingers_uk(TEST_GUESTS, TEST_DATE)),
        'electric_shuffle_london': ('Electric Shuffle (Canary Wharf)', lambda: electric_shuffle.scrape_electric_shuffle_london(TEST_GUESTS, TEST_DATE)),
        ...
        'f1_arcade': ('F1 Arcade (St Paul\'s)', lambda: f1_arcade.scrape_f1_arcade(TEST_GUESTS, TEST_DATE, f1_experience='Team Racing')),
        'topgolf_chigwell': ('Topgolf (Chigwell)', lambda: topgolfchigwell.scrape_topgolf_chigwell(TEST_GUESTS, TEST_DATE)),
        'hijingo': ('Hijingo (Shoreditch)', lambda: hijingo.scrape_hijingo(TEST_GUESTS, TEST_DATE)),
