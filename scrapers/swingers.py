...
#             scraper.wait_for_timeout(8000)  # Wait for calendar to render
            
#             # Wait for calendar dates to be available
#             try:
#                 scraper.wait_for_selector("li.slot-calendar__dates-item", timeout=30000)
#             except Exception:
#                 logger.warning("Calendar dates not found, continuing anyway...")
            
#             scraper.wait_for_timeout(3000)  # Additional wait for dynamic content
...
#             if not target_li:
#                 logger.info(f"No calendar entry found for {target_date}")
#                 return results
            
#             # Navigate to the date page
#             full_url = "https://www.swingers.club" + target_li.find("a")["href"]
#             scraper.goto(full_url, timeout=60000, wait_until="networkidle")
#             scraper.wait_for_timeout(8000)  # Wait for slots to load
            
#             # Wait for slot buttons to appear
#             try:
#                 scraper.wait_for_selector("button[data-day]", timeout=30000)
#             except Exception:
#                 logger.warning("Slot buttons not found, continuing anyway...")
...
def scrape_swingers_uk(guests, target_date):
    ...
    try:
        ...
        with BaseScraper() as scraper:
            ...
            # ---- LOAD PAGE WITH FORCED STOP (same as NYC) ----
            try:
                scraper.goto(url, timeout=4000, wait_until="domcontentloaded")
            except:
                pass
